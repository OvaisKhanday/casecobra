...
// webhook and thankyou (redirect) will happen simultaneously
// so we can't relay on the webhook
const ThankYou: FC<ThankYouProps> = async ({}) => {
  const searchParams = useSearchParams();
  const orderId = searchParams.get("orderId") || "";
  const { data } = useQuery({
    // on-page-load will immediately fire this
    ...
    <div className='bg-white'>
      <div className='mx-auto max-w-3xl px-4 py-6 sm:px-6 sm:py-24 lg:px-8'>
        <div className='max-w-xl'>
          ...
          <h1 className='mt-2 text-4xl font-bold tracking-tighter sm:text-5xl'>Your case is on the way!</h1>
        ...
        <div className='mt-10 border-t border-zinc-200'>
          <div className='mt-10 flex flex-auto flex-col'>
            ...
            <p className='mt-2 text-sm text-zinc-600'>
              We at casecobra believe that a phone case doesn't only need to look good, but also last you for the years to come. We offer a 5-year
              print guarantee. If your case isn't of the highest quality, we'll replace it for you.
            </p>
          </div>
        </div>
        ...
        <div className='space-y-6 border-t border-zinc-200 pt-10 text-sm'>
          ...
          <div className='flex justify-between'>
            ...
            <p className='font-medium text-zinc-800'>{formatPrice(amount)}</p>
          </div>
        </div>
      </div>
    </div>
