...
const DesignConfigurator: FC<DesignConfiguratorProps> = ({ configId, imageUrl, imageDimensions }) => {
  ...
  }>({ color: COLORS[0], model: MODELS.options[0], material: MATERIALS.options[0], finish: FINISHES.options[0] });
  ...
  async function saveConfiguration() {
    try {
      const { left: caseLeft, top: caseTop, width: caseWidth, height: caseHeight } = phoneCaseRef.current!.getBoundingClientRect();
      ...
      userImage.crossOrigin = "anonymous"; // don't get any error in source
      userImage.src = imageUrl;
      await new Promise((resolve) => (userImage.onload = resolve));
      ctx?.drawImage(userImage, actualX, actualY, renderedDimension.width, renderedDimension.height);

      const base64 = canvas.toDataURL();
      const base64Data = base64.split(",")[1]; // [0] metadata

      const blob = base64ToBlob(base64Data, "image/png");

      const file = new File([blob], `cropped__${configId}.png`, { type: "image/png" });

      await startUpload([file], { configId });
    } catch (error) {
      toast({
        title: "something went wrong",
        description: "There was a problem saving config, please try again",
        variant: "destructive",
      });
    }
  }
    ...
    <div className='relative mt-20 grid  grid-cols-1 lg:grid-cols-3 mb-20 pb-20'>
      ...
      >
        <div className='relative w-60 bg-opacity-50 pointer-events-none aspect-[896/1831] '>
          <AspectRatio ref={phoneCaseRef} ratio={896 / 1832} className='pointer-events-none relative z-50 aspect-[896/1831] w-full'>
        ...
        >
          <div className='relative w-full h-full'>
            <NextImage fill src={imageUrl} alt='user image' className='pointer-events-none' />
          </div>
        </Rnd>
      </div>
          ...
          <div className='px-8 pb-12 pt-8'>
            ...
            <div className='relative mt-4 h-full flex flex-col justify-between'>
              <div className='flex flex-col gap-6'>
                ...
                {[MATERIALS, FINISHES].map(({ title, options: selectableOptions }) => (
                  <RadioGroup
                    key={title}
                    value={options[title]}
                    onChange={(val) => {
                      setOptions((prev) => ({ ...prev, [title]: val }));
                    }}
                  >
                    <Label>{title.slice(0, 1).toUpperCase() + title.slice(1)}</Label>
